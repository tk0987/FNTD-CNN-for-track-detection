...
train_peak_dir=os.chdir(r"C:/Users/User/Desktop/spider_nn/data_generator/datanew/train/images/")
extension="*.bmp"
train_peaks=list()
...
train_mask_dir=os.chdir(r"C:/Users/User/Desktop/spider_nn/data_generator/datanew/train/masks/")
extension="*.bmp"
train_masks=[]
for file in glob.glob(extension):
...
test_peak_dir=os.chdir(r"C:/Users/User/Desktop/spider_nn/data_generator/datanew/test/images/")
extension="*.bmp"
test_peaks=[]
for file in glob.glob(extension):
   image=Image.open(file)
...
test_mask_dir=os.chdir(r"C:/Users/User/Desktop/spider_nn/data_generator/datanew/test/masks/")
extension="*.bmp"
test_masks=[]
for file in glob.glob(extension):
   image=Image.open(file)
...
# dense block 4
    ...
    x=tf.keras.layers.Add()([shortcut3c,shortcut3cy,shortcut3cz,x])
    x=tf.keras.layers.Conv2D(1,(1,1),(1,1),kernel_initializer=tf.keras.initializers.HeNormal(seed=seed+334),activation="relu")(x)
    x=tf.keras.layers.Add()([shortcut1c,shortcut1cy,shortcut1cz,x])
    x=tf.keras.layers.BatchNormalization()(x)
...
opti=tf.keras.optimizers.AdamW(0.0005,0.01)
print("kompilacja")
model.compile(optimizer=opti, loss=tf.keras.losses.BinaryFocalCrossentropy(gamma=2.5,alpha=0.01), metrics=[tf.keras.metrics.BinaryIoU(),tf.keras.metrics.BinaryAccuracy(),tf.keras.metrics.AUC()])

#training, callbacks

my_callbacks = tf.keras.callbacks.ModelCheckpoint(filepath=r'C:/Users/User/Desktop/spider_nn/fitterer/pc trained/cerberConvSigmoid.binaryfcross.g3.5.al0.5.1b.4n.Epoka-{epoch:02d}_loss-{val_loss:.4f}_IoU-{val_binary_io_u:.4f}_binAcc-{val_binary_accuracy:.4f}.h5',verbose=1,monitor="val_loss",save_weights_only=False,mode="min",save_best_only=True)
print("fitowanie")
model.fit(datagen.flow(train_peaks, train_masks,batch_size=1), batch_size=1, epochs=200, verbose=1,callbacks=my_callbacks,validation_data=(test_peaks, test_masks),validation_batch_size=1)